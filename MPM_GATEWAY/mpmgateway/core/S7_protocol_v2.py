...
class S7_protocol(ODBC_gateway):
    ...
    def get_data(self):
        """
        Connects to Siemens S7 PLC via snap7, reads all configured tags,
        and stores results in self.data dict.
        """
        self.data = {}
        data_dict = {}

        if self.write_data:
            return  # write mode — skip reading

        self.instructions = self.read_instructions

        client = snap7.client.Client()
        try:
            client.connect(self.PLC_IP_ADDRESS, self.PLC_RACK, self.PLC_SLOT)

            if not client.get_connected():
                print("Failed to connect to the PLC.")
                return

            print(f"Connected to Siemens PLC @ {self.PLC_IP_ADDRESS}")

            for instruction in self.instructions:

                content    = instruction.get("content", "UNKNOWN")
                storage    = instruction.get("storage")       # "DB" or "MK"
                data_type  = instruction.get("type")          # "real", "int", "bool"

                try:
                    # ── WRITE instruction ──────────────────────────────────────
                    if "value_to_write" in instruction:
                        self.write_slc_data(self.plc,
                                            instruction["address"],
                                            instruction["value_to_write"])
                        print(f"  [WRITE] {content} → {instruction['value_to_write']}")
                        continue

                    # ── READ from DB ───────────────────────────────────────────
                    if storage == "DB":
                        DB_NUMBER = instruction["DB_no"]

                        if data_type == "bool":
                            byte_idx = instruction["byte"]
                            bit_idx  = instruction["bit"]
                            raw = client.read_area(snap7.type.Areas.DB, DB_NUMBER, byte_idx, 1)
                            print(f"  [RAW] {content} DB={DB_NUMBER} byte={byte_idx} bit={bit_idx} hex={raw.hex()}")
                            value = get_bool(raw, 0, bit_idx)

                        elif data_type in ("real", "int"):
                            START_ADDRESS = instruction["address"]
                            # enforce correct SIZE regardless of config typo
                            SIZE = 4 if data_type == "real" else 2
                            raw = client.read_area(snap7.type.Areas.DB, DB_NUMBER, START_ADDRESS, SIZE)
                            print(f"  [RAW] {content} DB={DB_NUMBER} addr={START_ADDRESS} size={SIZE} hex={raw.hex()}")

                            if len(raw) < SIZE:
                                print(f"  [WARN] {content}: expected {SIZE} bytes, got {len(raw)} — skipping")
                                continue
                            ...
                            print(f"  [SKIP] {content}: unknown type '{data_type}'")
                            continue

                    # ── READ from MK (Merker/Flag area) ───────────────────────
                    elif storage == "MK":
                        START_ADDRESS = instruction["address"]
                        SIZE = 4 if data_type == "real" else 2
                        raw = client.read_area(snap7.type.Areas.MK, 0, START_ADDRESS, SIZE)
                        print(f"  [RAW] {content} MK addr={START_ADDRESS} size={SIZE} hex={raw.hex()}")
                        ...
                        if data_type == "real":
                            ...
                            value = get_bool(raw, 0, instruction["bit"])
                        else:
                            print(f"  [SKIP] {content}: unknown type '{data_type}'")
                            continue

                    else:
                        print(f"  [SKIP] {content}: unknown storage '{storage}'")
                        continue

                    # ── Store result ───────────────────────────────────────────
                    if isinstance(value, float):
                        data_dict[content] = round(value, 2)
                    else:
                        data_dict[content] = value

                    print(f"  [OK]  {content}: {data_dict[content]}")

                except Exception as tag_err:
                    # one bad tag should not kill the entire read cycle
                    print(f"  [ERROR] {content}: {tag_err}")
                    data_dict[content] = None

            self.data = data_dict

        except Exception as e:
            print(f"[FATAL] PLC connection error: {e}")
    ...
    def send_data_to_db(self):
        ...
        insert_query = f"INSERT INTO {self.storing_table} \
            ({', '.join([f'`{col}`' for col in self.data.keys()])}) \
            VALUES ({', '.join(['?'] * len(self.data.keys()))})"
        try:
            # Convert bool → int for MySQL TINYINT(1), keep float/None as-is
            values = [
                int(v) if isinstance(v, bool)
                else v  # float, int, None all pass through directly
                for v in self.data.values()
            ]
            self.storing_db_cursor.execute(insert_query, values)
            logging.info("stored the data in destination table successfully")
        except Exception as e:
            logging.error(f"Error while storing the data in the destination table: {e}")
