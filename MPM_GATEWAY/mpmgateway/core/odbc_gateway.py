...
class ODBC_gateway():
    ...
    def etl_process(self,etl_file_path, arg1):
        ...
        result = getattr(module, function_name)(arg1,self.etl_config)
        return result
    ...
    def duplicate_db_source_to_local(self):
        ...
        while True:
            try:
                ...
                try:
                    sdb_conn = self.connect_to_db(self.sdb_conn_str)
                    logging.info(f"Connection to source database [connection string : {self.sdb_conn_str}] : successful")
                except Exception as e:
                    logging.error(f"Connection to source database failed [connection string : {self.sdb_conn_str}] ,error : {e}")
                ...
                # self.data = self.extract_data_from_db()
                # if self.data:
