...
class ODBC_gateway():
    ...
    def __init__(self,config,etl_config = None):
        ...
        log_folder =  self.log_file_path
        log_filename = datetime.datetime.now().strftime("%Y-%m-%d.log")
        self.log_filepath = os.path.join(log_folder, "odbc_connection_" + log_filename )
        ...
        # source table
        self.source_table2 = config['source_database']['table_name2']
        self.source_table_schema2 = config['source_database']['schema2']
        self.columns_to_fetch2 = config['source_database']['columns_needed2']
        self.filter_id2 = config['source_database']['filter_id2']
        self.file_keyword2 = config['source_database']['file_keyword2']
    ...
    def get_max_id(self):
        ...
        :rtype: str or int
        """
        return max([i.__getattribute__(self.filter_id) for i in self.data])
    ...
    def fetch_ids_need_processing_v2(self,filter_id,source_table):
        """it fetchs ids that need processing
        ...
        """
        try:
            self.fetching_db_cursor.execute(f"SELECT DISTINCT Date({filter_id}) FROM {source_table}")
            all_files = set(row.__getattribute__(f"Date({filter_id})") for row in self.fetching_db_cursor.fetchall())
        except Exception as e:
            logging.error(e)
            raise e
    ...
    def extract_data_from_db_v3(self,colummns_to_fetch,filter_id,source_table):
        """extracts data from the source database using list of ids that need processing

        :return:  data , column names
        :rtype: 2d array, list
        """
        columns_str = ', '.join(f"`{i}`"for i in colummns_to_fetch)
        ids = self.fetch_ids_need_processing_v2(filter_id,source_table)
        if len(ids) == 0:
            logging.info(f'No new data to store in the destination table')
            return None,None
        else:
            fetch_query = f"SELECT {columns_str} FROM {source_table} WHERE Date({filter_id}) IN ({', '.join(['?'] * len(ids))})"
            try:
                result = self.fetching_db_cursor.execute(fetch_query,list(ids))
                data = result.fetchall()
                if not data:
    ...
    def extract_data_from_db(self):
        ...
        last_id,_ = self.fetch_last_id_from_idlogger()
        if last_id:
    ...
    def send_data_to_db(self):
        """it sends the processed data or the replicated data to the database
        """
        insert_query ="INSERT INTO scada_data (`datetime`, `Bentonite_actual_value`, `Bentonite_set_value`, `Fines_actual_value`, `Fines_set_value`, `return_sand_timer`, `return_sand_capacity_actual`, `return_sand_capacity_set`, `set_compactability`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        data = ['2024-07-26 11:19:55.595147', '20.0', '21.0', '6.0', '7.0', '220.0', '1799.0', '0.0', '16928.0']


        self.storing_db_cursor.execute(insert_query,['2024-07-26 11:19:55.595147', '20.0', '21.0', '6.0', '7.0', '220.0', '1799.0', '0.0', '16928.0'])
        ...
        #         self.storing_db_cursor.execute(insert_query,list(map(str,list(self.data[row]))))
    ...
    def get_dataframe(self):
        ...
        return df
    def get_dataframe_v2(self,data,column_names):
        """Transform the 2d array to dataframe 
    ...
    def etl_process(self,etl_file_path, arg1,arg2):
        ...
        result = getattr(module, function_name)(arg1,arg2,self.etl_config)
        return result
    ...
    def etl_server_to_sandman(self):
        ...
        try:
            ...
            self.data,self.column_names = self.extract_data_from_db_v2()

            # processing and sending data to the server
            if self.data:
                self.data_to_process = self.get_dataframe()   
                if self.etl:
                    try:
                        self.data_processed = self.etl_process(self.etl_path, self.data_to_process)
                        logging.info(f'ETL completed')
                        logging.info(f'The files to be processed along with the filename and row count : {self.data_processed["file_name"].value_counts()}')
                    except Exception as e:
    ...
    def etl_processing_v2(self):
        ...
        try:
            ...
            # extract data from the source database
            self.data,self.column_names = self.extract_data_from_db_v3(self.columns_to_fetch,self.filter_id,self.source_table)
            self.data2,self.column_names2 = self.extract_data_from_db_v3(self.columns_to_fetch2,self.filter_id2,self.source_table2)

            # processing and sending data to the server
            if self.data and self.data2:
                self.data_to_process = self.get_dataframe_v2(self.data,self.column_names)  
                self.data_to_process2 = self.get_dataframe_v2(self.data2,self.column_names2)  

                if self.etl:
                    try:
                        self.data_processed = self.etl_process(self.etl_path, self.data_to_process,self.data_to_process2)
                        logging.info(f'ETL completed')
                        logging.info(f'The files to be processed along with the filename and row count : {self.data_processed["file_name"].value_counts()}')
                    except Exception as e:
                        logging.error(f'Error while processing the data : {e}')
