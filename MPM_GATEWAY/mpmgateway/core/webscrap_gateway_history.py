...
class Scrapper():
    ...
    """
    def __init__(self,config,mould_config,etl_config = None):
        ...
        """
        self.mould_config = mould_config
        ...
        self.filter_id = config['destination_database']['filter_id']
    ...
    def etl_process(self,etl_file_path, arg1,arg2):
        ...
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        result = getattr(module, function_name)(arg1,arg2,self.mould_config)
        return result
    ...
    # def get_data(self):
    #     """
    #     Fetches data from the provided URL without requiring login credentials.
    #     """
    #     protected_url = self.scrapping_urls["data"]  # URL to scrape

    #     try:
    #         # Directly access the URL without login
    #         logging.info("Accessing data page without login...")
    ...
    #         else:
    #             logging.error(f"Failed to access the page. Status Code: {response.status_code}")
    ...
    def get_data(self):
        ...
        if login_response.ok:
            ...
            base_url = self.scrapping_urls["data"]
            ...
            url = f"{base_url}&start={start_date}&stop={stop_date}&order=asc&lang=en-GB&skip={skip}&take={take}"
            response = session.get(url)
            if not response.ok:
                logging.error("Initial data fetch failed")
                return

            data = response.json()
            total_count = data.get("iterator", {}).get("count", 0)
            logging.info(f"Total records to fetch: {total_count}")

            while skip < total_count:
                paginated_url = f"{base_url}&start={start_date}&stop={stop_date}&order=asc&lang=en-GB&skip={skip}&take={take}"
                page_response = session.get(paginated_url)

                if page_response.ok:
                    json_data = page_response.json()
                    df = self.etl_process(self.etl_config, json_data, self.last_id)
                    all_df.append(df)
                    logging.info(f"Fetched records at skip={skip}")
                else:
                    logging.warning(f"Failed to fetch page at skip={skip}")
                    break
            ...
            if all_df:
                self.data = pd.concat(all_df, ignore_index=True)
            else:
